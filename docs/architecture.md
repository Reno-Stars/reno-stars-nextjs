...
components/
  ...
  home/                   # Homepage section components (15 files: Hero, ServiceAreas, Testimonials, GoogleAvatar, Gallery, Services, Stats, About, TrustBadges, Partners, FAQ, Blog, Showroom, Contact, Marquee)
...
- **Homepage sections** (`components/home/`): 15 section components extracted from HomePage for code-splitting. 10 are Server Components (HeroSection, ServiceAreasBar, TestimonialsSection, ServicesSection, StatsSection, AboutSection, TrustBadgesSection, PartnersSection, BlogSection, ShowroomSection), 3 are Client Components (GallerySection, FaqSection, ContactSection), 1 is a Client utility (GoogleAvatar), 1 is a shared Client component (Marquee). TestimonialsSection and PartnersSection use the `<Marquee>` client component for infinite horizontal scroll. FaqSection uses CSS Grid accordion animation (`grid-template-rows: 0fr` → `1fr`). Below-fold sections are loaded via `next/dynamic` with skeleton fallbacks.
- **Marquee** (`components/home/Marquee.tsx`): Reusable client component for infinite-scroll carousels used by TestimonialsSection and PartnersSection. Accepts `children` (semantic items) and `repeatCount` (number) as props. Semantic items are always in SSR output for crawlers. Clones are created client-side via DOM `cloneNode(true)` in a `useEffect` — no React elements are serialized in the RSC hydration payload, so each item appears exactly once in the raw HTML. The effect snapshots `semanticRef.children` to a static array, then appends `repeatCount * 2 - 1` clone sets to a separate `clonesRef` container (marked `aria-hidden` + `inert`). Animation starts only after clones mount (via inline `style` with `marquee-scroll` keyframes from `globals.css`). Respects `prefers-reduced-motion` with a live `matchMedia` listener — clones and animation are removed if the user toggles the setting. Hover and focus pause via `onMouseEnter`/`onMouseLeave`/`onFocus`/`onBlur` event handlers. Parent server components calculate `repeatCount` to ensure one set of items fills ≥2000px.
...
- **ContactForm**: Reusable form component with `large` prop for accessibility (larger text/inputs for elderly users). Tracks success timeout via `useRef` with cleanup on unmount. Surfaces server error messages. Required fields have `aria-required="true"` and all inputs reference the error alert via `aria-describedby` when an error is shown.
