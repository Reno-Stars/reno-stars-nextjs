...
components/
  ...
  structured-data/        # JSON-LD schema components (11 schemas)
...
| `getProjectSlugsFromDb()` | `{ slug, updatedAt }[]` | Published slugs + dates for sitemap |
...
- **Structured data** (`components/structured-data/`): JSON-LD schema injected via `<script type="application/ld+json">`. Used in server page route files. Schema components accept `company` as a prop. Includes 11 schemas: LocalBusinessSchema, LocalBusinessAreaSchema, WebSiteSchema (with `SearchAction` for sitelinks search), ServiceSchema, ProjectSchema (`WebPage` with `mainEntity: Service`), ArticleSchema (with `ImageObject` including width/height), BreadcrumbSchema, FAQSchema, ReviewSchema, HowToSchema (process page, 5-step renovation workflow with tools and total time). Rating data comes from Google Reviews API (`googleRating`/`googleReviewCount` props) — no longer stored in the Company model. `aggregateRating` is conditionally rendered only when Google data is available. ReviewSchema emits individual reviews only (aggregate rating handled globally by LocalBusinessSchema in the layout).
