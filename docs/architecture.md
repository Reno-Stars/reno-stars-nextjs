...
app/                      # Next.js App Router
  [locale]/               # Dynamic locale segment (en | zh)
    ...
    workflow/             # 5-step renovation workflow page
    about/                # About Us page (company story, values, FAQ, OrganizationSchema)
    showroom/             # Showroom page
    guides/               # Renovation cost guides hub + individual guides
    reviews/              # Reviews/Testimonials page (Google Reviews, force-dynamic)
...
components/
  ...
  structured-data/        # JSON-LD schema components (13 schemas)
  ...
  TetrisGallery.tsx       # Masonry gallery layout (exports `tetrisLayouts` for shared use)
  OptimizedImage.tsx      # Responsive image with srcSet via /api/image (WebP, sharp)
  illustrations/          # Extracted SVG illustration components
    process.tsx           # 5 step illustrations + 2 decorative SVGs for ProcessPage
...
lib/
  db/                     # Database layer
    ...
    seed.ts               # Seed script
  image.ts                # Shared image optimization utilities (SRCSET_WIDTHS, buildOptimizedUrl, buildSrcSet, buildPreloadUrl)
  ...
  admin/                  # Admin utilities
    ...
    form-utils.ts         # Form validation + image pair parsing (getString, isValidUrl, parseImagePairs, validatePairUrls, validateExternalProductUrls, etc.)
...
| `getServicesFromDb()` | `Service[]` | Orders by `displayOrder`, applies `getAssetUrl()`, batch-fetches tags and benefits via `groupBy()`, maps `showOnServicesPage` and `isProjectType` flags |
...
| `getDesignsFromDb()` | `DesignItem[]` | Published design renderings, `getAssetUrl()` applied to images |
...
| `getKitchenProjectsForGuide()` | `KitchenGuideProject[]` | Published kitchen projects with budget, duration, space type (EN+ZH) |
| `getBathroomProjectsForGuide()` | `KitchenGuideProject[]` | Published bathroom projects (same interface as kitchen) |
| `getWholeHouseProjectsForGuide()` | `KitchenGuideProject[]` | Published whole-house projects (same interface as kitchen) |

#### Service Visibility Filtering

Services have two independent boolean flags (`showOnServicesPage`, `isProjectType`) that control where they appear:

| Flag | Controls | Filtering location |
|------|----------|-------------------|
| `showOnServicesPage` | Services page, service detail pages, homepage, footer, sitemap | Layout (footer), pages, sitemap |
| `isProjectType` | Navbar dropdown, project filter buttons, `?service=` validation | Layout (navbar), `getServiceTypeMap()` |

`getServiceTypeMap()` (in `lib/db/queries.ts`) skips services with `isProjectType === false`. Since `getCategoriesLocalized()` and `getCategorySlugs()` consume this map, they automatically exclude non-project services. Public pages use defensive `!== false` checks to handle both `undefined` (non-DB sources) and `false`.

Admin-only (uncached) query functions: `getAllProjectsAdmin()`, `getAllServicesAdmin()`, `getAllBlogPostsAdmin()`, `getAllContactsAdmin()`, `getAllSocialLinksAdmin()`, `getAllServiceAreasAdmin()`, `getAllDesignsAdmin()`, `getAllTrustBadgesAdmin()`, `getAllFaqsAdmin()`, `getAllPartnersAdmin()`, `getAllSitesAdmin()`, `getAllSocialMediaPostsAdmin()`, `getSocialMediaPostByIdAdmin()`.
...
3. Renders `<Navbar>` (receives `company`, `services` filtered by `isProjectType !== false`) and `<Footer>` (receives `company`, `socialLinks`, `services` filtered by `showOnServicesPage !== false`, `areas`, `googleRating`) with this data as props
...
// Image pairs support optional video per side:
interface ImagePair {
  beforeImage?: ImageWithAlt;
  afterImage?: ImageWithAlt;
  beforeVideo?: string;   // Video URL (optional)
  afterVideo?: string;    // Video URL (optional)
  title?: Localized<string>;
  // ...
}

// Projects and sites support hero video:
interface Project {
  hero_image: string;
  hero_video?: string;    // Hero video URL (optional)
  // ...
}
...
| Surface Light | `#f5f0e8` | Lighter surface for gradients |
| Surface Dark | `#e0d6c8` | Darker surface for gradients |
...
- **Page components** (`components/pages/`): One per route. All are `'use client'` components. Receive `locale`, `company`, and page-specific data as props. Do **not** render Navbar or Footer. `AreaPage` receives area-specific FAQs, DB-sourced projects, and renders unique content/highlights when available from the database. `ServicesPage` receives `areas` and renders area city cards below the services grid. `ServiceLocationPage` is enriched with 11 sections: Hero, service description, area content (unique per city from `localizedArea.content`, split by `\n\n`), service tags, "Why Choose Us" benefits (uses `localizedArea.highlights` when available, falls back to generic benefits), related projects (DB-sourced via `getProjectsByAreaFromDb`, prioritized by service type), FAQ accordion, workflow link, "Other Areas" cross-links, "Other Services in Area" cross-links, and CTA. Receives optional `services`, `areas`, `faqs`, and `areaProjects` props. Meta titles use service tags for SERP differentiation (first 2 tags joined with " & ", only when ≤ 60 chars). `BlogPostPage` includes "Related Services" and "Related Areas" cross-linking sections after article content for internal link building. `ProcessPage` includes scroll-driven road SVG animation and poster PNG/PDF export — uses a width-locking pattern before `toPng` capture to fix `foreignObject` centering in `html-to-image`. SVG illustrations extracted to `components/illustrations/process.tsx`.
- **Homepage sections** (`components/home/`): 14 section components extracted from HomePage for code-splitting. 9 are Server Components (TestimonialsSection, GallerySection, ServicesSection, StatsSection, AboutSection, TrustBadgesSection, PartnersSection, BlogSection, ShowroomSection), 3 are Client Components (HeroSection, FaqSection, ContactSection), 1 is a Client utility (GoogleAvatar), 1 is a shared Client component (Marquee), 1 is a shared utility module (marquee-utils). HeroSection is a client component with a two-slide text slideshow (renovation + real estate photography) using `useMemo`/`useState`/`useCallback`/`useRef`: auto-advances every 6s with a 300ms fade transition, `currentRef` prevents stale closures in the interval, `aria-label` provides a stable section name, `aria-live="polite"` announces slide changes to screen readers, and slide indicator dots use `role="tablist"`/`role="tab"`. Hero image uses `OptimizedImage` with `priority` for fast LCP; `LazyVideo` overlays on desktop only (CSS `hidden md:block`). WorkSafe BC logo uses a plain `<img>` (local asset, no optimization needed). GallerySection displays featured project images (not designs) with links to project detail pages. TestimonialsSection and PartnersSection use the `<Marquee>` client component for infinite horizontal scroll. ContactSection includes a Google Maps embed (using shared `MAP_EMBED_URL` from `lib/data`). FaqSection uses CSS Grid accordion animation (`grid-template-rows: 0fr` → `1fr`); `subtitle` prop is optional (omitted on area pages). Below-fold sections are loaded via `next/dynamic` with skeleton fallbacks.
...
- **Structured data** (`components/structured-data/`): JSON-LD schema injected via `<script type="application/ld+json">`. Used in server page route files. Schema components accept `company` as a prop. All components sanitize JSON output with `.replace(/</g, '\\u003c')` to prevent XSS via `</script>` injection. Includes 13 schemas: LocalBusinessSchema, LocalBusinessAreaSchema, WebSiteSchema (with `SearchAction` for sitelinks search), ServiceSchema, ProjectSchema (`WebPage` with `mainEntity: Service`), ProjectCategorySchema (`ItemList` with positioned project links), ArticleSchema (with `ImageObject` including width/height), BreadcrumbSchema, FAQSchema, ReviewSchema, HowToSchema (workflow page, 5-step renovation workflow with tools and total time), ContactPageSchema (`ContactPage` with `HomeAndConstructionBusiness` + `ContactPoint`), OrganizationSchema (`HomeAndConstructionBusiness` with area served, offer catalog — used on About page, accepts optional `socialLinks`/`areas` props). Shared `parseAddress()` utility in `parse-address.ts` used by `LocalBusinessSchema`, `ContactPageSchema`, and `OrganizationSchema`. Rating data comes from Google Reviews API (`googleRating`/`googleReviewCount` props) — no longer stored in the Company model. `aggregateRating` is conditionally rendered only when Google data is available. ReviewSchema emits individual reviews only (aggregate rating handled globally by LocalBusinessSchema in the layout).
- **Navbar**: Unified navigation with 10 links (Home, Services, Projects, About, Design, Benefits, Workflow, Reviews, Contact, Blog & News). Receives `company` and `services` (filtered by `isProjectType !== false`) as props from layout. Uses `useMemo` for link arrays, focus trap for mobile menu, `aria-current="page"` on active links. Logo uses `priority` for faster LCP. Toggle state setters use functional updater form. Language switcher uses `<Link>` with `locale` prop for crawlable alternate-language discovery. **Projects dropdown**: The "Projects" item renders a hover-activated dropdown (desktop) or indented sub-items (mobile) listing all service categories. Links navigate to `/projects?service=<slug>` using `router.push()` from next-intl (native `<a>` tags with `href` fallback for no-JS). Desktop dropdown has `DROPDOWN_CLOSE_DELAY_MS` (150ms) close delay, ArrowDown/ArrowUp/Escape keyboard navigation, `role="menu"`/`role="menuitem"` ARIA, and `dropdownItemsRef` array (resized per render to prevent stale refs). Mobile uses semantic `<ul role="list">`/`<li>`. `NavLink` interface types the link array. Shared `isLinkActive()` callback unifies active-state logic between desktop and mobile. Outside-click listener only attached when dropdown is open; timeout cleared on unmount.
- **Footer**: 5-column grid (Brand & Social, Quick Links, Services, Contact, Why Us) + full-width Service Areas bar with city links. Section headings use `<h2>` for proper heading hierarchy. Receives `company`, `socialLinks`, `services` (filtered by `showOnServicesPage !== false`), `areas`, `googleRating` as props from layout. Custom SVG icons for Xiaohongshu, WeChat, WhatsApp, Linktree. WeChat button opens `WechatModal` — extracted component with Escape-to-close, auto-focus, and focus restoration to trigger button.
...
- **Projects page category filtering**: `ProjectsPage` filters by `service_type` slug (e.g., `"kitchen"`, `"bathroom"`) — not by localized category name. Categories from `getCategoriesLocalized()` include a `serviceType` field matching the project's `service_type` column. The `matchesCategory(project, serviceType)` helper handles the `'whole-house'` special case (matches `isSiteProject || service_type === 'whole-house'`). Both the "Browse by Category" cards and the dropdown filter use this slug-based matching. Supports `?service=<slug>` URL parameter for deep-linking to a pre-filtered category (e.g., from the navbar dropdown). The server component (`app/[locale]/projects/page.tsx`) validates the `service` search param against known categories (whitelist) and passes the validated `initialService` to the client component. `ProjectsPage` initializes `activeCategory` from `initialService` and syncs on prop changes via `useEffect` with an `isFirstRender` ref guard (handles client-side navigations where the component is reused without remounting).
...
- **Before & After page pagination**: `BeforeAfterGalleryPage` paginates filtered gallery items with `ITEMS_PER_PAGE = 12`. Same pagination pattern as ProjectsPage — smart page numbers with ellipsis, neumorphic Previous/Next buttons and numbered page controls. Resets to page 1 on service-type filter change. Smooth-scrolls to gallery grid on page change via `galleryRef`. Translation keys under `beforeAfterPage.pagination` namespace (EN + ZH).
...
- **Service visibility flags**: Two independent boolean columns on `services` — `showOnServicesPage` (controls services page, homepage, footer, sitemap) and `isProjectType` (controls navbar dropdown, project filters). Both default to `true`. Admin ServiceForm exposes these as checkboxes under a "Visibility" `FormField` with bilingual labels (`t.services.showOnServicesPageLabel`, `t.services.isProjectTypeLabel`). Server actions parse checkbox values via `formData.get('name') === 'on'`.
...
- **Hero slideshow a11y**: Section uses stable `aria-label` (not `aria-labelledby` on rotating h1). Slide content wrapper has `aria-live="polite"` for screen reader announcements. Slide indicator dots use `role="tablist"` with `role="tab"` buttons (`aria-selected`, `aria-label`).
...
- **Lightbox accessibility**: Design page lightbox includes `role="dialog"`, `aria-modal="true"`, keyboard navigation (Escape to close, ArrowLeft/ArrowRight for prev/next), body scroll lock with cleanup, and an image counter. Translation keys use `lightbox.*` namespace (shared with ProjectModal).
...
- **Video in galleries**: `ProjectDetailPage`, `SiteDetailPage`, and `ProjectModal` support video per pair side. When a pair side has a video URL, `<video controls playsInline>` is rendered with `poster={displayImage?.src}`. Hero video (project/site walkthrough) renders above the gallery as a full-width `<video controls>` element. Video-only pairs display a `Video` icon placeholder in thumbnail strips.
...
Both batch upload and admin form AI generate pass `availableServiceTypes` from the DB (`getServiceTypeMap()`) and all scopes to `optimizeProjectDescription()`. The review prompt's valid service type list is built dynamically via `buildReviewPrompt(availableServiceTypes)`. Batch upload passes type-specific scopes when `serviceType` is known, or `ALL_SCOPES` (deduplicated) when unknown. The `AVAILABLE_SCOPES` prompt includes both EN and ZH names (e.g., `Flooring (地面工程)`) so the AI can match Chinese notes to the correct English scope name. The save step resolves AI-selected scope names via the shared `SCOPE_EN_TO_ZH` lookup across all service types, falling back to type-specific scopes if all AI names were invalid. `fillMissingSeoFields()` also runs on site descriptions via `optimizeSiteDescription()`.
...
Accepts query params: `url` (source), `w` (width), `q` (quality, default 75), `f` (format: `webp`/`avif`). Validates `url` against an allowlist (`reno-stars.com`, R2 buckets, `googleusercontent.com`; localhost/127.0.0.1 in dev only). SSRF protection: uses `redirect: 'follow'` with post-redirect hostname validation (rejects redirects to disallowed hosts). Rejects sources >50MB via both `Content-Length` header and post-download `byteLength` check. Snaps requested width to the nearest value in `SRCSET_WIDTHS` ([320, 640, 828, 1080, 1200, 1920]). Returns optimized image with `Cache-Control: public, max-age=31536000, immutable`. In-memory LRU cache (50 entries, 1hr TTL) for long-lived processes; on serverless the CDN/browser cache is the primary layer.

### Shared Utilities (`lib/image.ts`)

Exports `SRCSET_WIDTHS`, `buildOptimizedUrl(src, width, quality?, format?)`, `buildSrcSet()`, and `buildPreloadUrl()` — shared between `OptimizedImage` component and the API route for consistent width breakpoints. `buildOptimizedUrl` accepts an optional `format` parameter (`'webp' | 'avif'`, defaults to `'webp'`).

### OptimizedImage Component (`components/OptimizedImage.tsx`)

Client component. Drop-in replacement for `next/image` used by all 18 public components (Navbar, Footer, ProjectCard, ProjectModal, TetrisGallery, HeroSection, StatsSection, ShowroomSection, and all page components). Generates native `<img>` with responsive `srcSet` pointing to `/api/image`. For external URLs: builds srcSet at 6 breakpoints, sets `fetchPriority="high"` when `priority` is true, falls back to original `src` via `onError` (guards against infinite loop with `data-fallback` attribute). For local paths (e.g. `/worksafe-bc-logo.jpg`): renders a plain `<img>` without optimization. Supports `fill` mode (absolute positioning + `object-fit: cover`). No `next/image` imports remain in the codebase.

### LazyVideo Component (`components/LazyVideo.tsx`)

Always renders `<video>` element with CSS `hidden md:block` to avoid CLS from JS-driven conditional rendering. On desktop, uses IntersectionObserver to lazy-load video source when visible. Handles device rotation via `matchMedia('change')` listener. Respects `prefers-reduced-motion`.

### Layout Preload (`app/[locale]/layout.tsx`)

Preloads the hero image as an optimized 828px WebP via `buildPreloadUrl()` with `type="image/webp"`. Browsers without WebP support (<3%) skip the preload; the `OptimizedImage` `onError` fallback still loads the original image.
...
The modal uses `imagePairs[]` / `activePairIndex` / `showBefore` state instead of a flat image array. Each pair can have a `beforeImage` and/or `afterImage`, plus optional `beforeVideo` and/or `afterVideo`. Users click the main image to toggle between before/after views when both sides exist. When a pair side has a video, `<video controls>` is rendered instead of `<Image>`.
...
Split before/after preview per pair (matching detail page UX). Each thumbnail shows both images side-by-side with a white divider line. Responsive sizing: `h-[30px] sm:h-[48px]`. Three-branch rendering: (1) split before/after images, (2) single image, (3) video-only placeholder with `Video` icon on navy background. Width uses `hasBothSides` which checks for both images and videos.
