...
- **VideoUrlInput** (`components/admin/VideoUrlInput.tsx`): Same pattern as ImageUrlInput but for video files. Accepts MP4/WebM/MOV via `VIDEO_ACCEPT` constant (max 1 GB). Shows `<video controls preload="metadata">` preview. Uses `uploadVideoDirect()`. Supports `onChange` callback for controlled mode and `compact` prop for smaller UI when embedded inside other editors (e.g., ImagePairEditor). Used by CompanyForm (hero video), ProjectForm (hero video), SiteForm (hero video), and ImagePairEditor (before/after video per pair).
