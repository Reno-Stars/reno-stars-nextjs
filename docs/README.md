...
- **Database-driven:** Company info, services, social links, about sections, projects, service areas, blog posts, designs, trust badges, partners, and showroom info fetched from PostgreSQL via cached query layer (`lib/db/queries.ts`). Homepage testimonials powered by Google Places API (`lib/google-reviews.ts`) with 24h caching
- **Admin dashboard:** Auth-protected CRUD for all 14 content types at `/admin/` with unified House Stack UI for site/project management (sites, projects, blog, social posts, contacts, company, services, social links, service areas, designs, trust badges, partners, showroom, about sections, FAQs). AI-powered content optimization via OpenAI for blog posts, project descriptions, and social media post generation (Instagram, Facebook, Xiaohongshu from any source). One-click blog generation from project/site data (GPT-4o with zod validation, SEO field truncation, slug sanitization). All image uploads use presigned S3 URLs (up to 50 MB per image, 1 GB for batch ZIP). Video uploads auto-compressed client-side via ffmpeg.wasm (CRF 18 H.264) before upload. Batch upload: ZIP of renovation images auto-creates sites, projects, image pairs, and blog posts with AI-generated metadata. Supports two modes: Sites (whole-house with containers) and Standalone (individual projects). Blog posts can link to related projects to display products used
- **Whole House projects:** Sites aggregate child projects with combined budget, duration, service scopes, and external products for "Whole House" renovation display. Both projects and sites use before/after image pairs with full SEO metadata (bilingual titles, captions, alt text, photographer credit, keywords). Projects and sites support hero video and per-pair before/after video alongside images
- **Enriched area pages:** 14 service area pages with unique bilingual content, custom SEO metadata, area-specific FAQs with structured data, custom benefits/highlights, and DB-sourced related projects — eliminating thin/doorway page risk
...
- **Accessible:** Skip-to-content link, elderly-friendly large text mode for contact forms, responsive hero scaling, valid heading hierarchy, design page lightbox with `role="dialog"` and keyboard navigation, keyboard focus traps, fullscreen modal a11y (scroll lock, focus trap, Escape/Arrow keys via `useFullscreenModal` hook), `prefers-reduced-motion` support, public error boundary with `role="alert"`
...
- **Well-tested:** 451 unit tests across 26 test files (Vitest), covering admin components, hooks, actions, and utilities
