...
- **SEO-optimized:** Dynamic sitemap with per-entry timestamps, JSON-LD structured data (11 schema types including WebSite, FAQ, HowTo), OG images with dimensions on all pages, `og:locale:alternate` for bilingual social signals, database-driven SEO metadata, 50+ WordPress redirects, meta description truncation, security headers via proxy
