...
async function seed() {
  ...
  if (existingAbout.length === 0) {
    await db.insert(aboutSections).values({
      ...
      whyChooseUsEn: `Licensed, insured with ${COMPANY_STATS.liabilityCoverage} WCB liability coverage, and backed by up to a 3-year warranty. Our 5-star Google rating and dedicated team of ${COMPANY_STATS.teamSize}+ professionals ensure your project is in expert hands.`,
      whyChooseUsZh: `持证经营，拥有至多${COMPANY_STATS.liabilityCoverage.replace('Up to ', '')}WCB责任保险和长达3年保修。我们的Google五星好评和${COMPANY_STATS.teamSize}人专业团队确保您的项目由专家负责。`,
  ...
  if (existingFaqs.length === 0) {
    await db.insert(faqs).values([
      ...
      },
      {
        ...
        answerZh: `Reno Stars 始终致力于提供高品质的工艺，拥有超过${getYearsExperience()}年的综合经验。我们拥有WorkSafe BC至多${COMPANY_STATS.liabilityCoverage.replace('Up to ', '')}责任保障、所有工程长达3年质保、透明定价，已成功完成${COMPANY_STATS.projectsCompleted}项目。我们的Google五星好评反映了我们对客户满意度的承诺。`,
        displayOrder: 2,
      },
      {
