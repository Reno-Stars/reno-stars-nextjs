...
export interface GoogleReview {
   ...
   * @deprecated 2026-05-28 — use `translations.zh` instead. Kept for backward
   * compat during the migration so old cache payloads still render correctly.
   * The cache-update script writes BOTH fields during the transition.
   */
  textZh?: string;
  /**
   * Per-locale translations of review text, populated by the
   * `scripts/update-reviews-cache.ts` translation pass. Read by
   * `TestimonialsSection` as `translations?.[locale] ?? text` (EN fallback).
   *
   * Hongming auth 2026-05-28: one-time + recurring per-review LLM translation
   * cost (~$0.14 one-shot for ~10 reviews × 14 locales) is approved.
   * One-shot per review (write-once, don't re-translate on every cache pull).
   *
   * Keys are BCP-47 codes matching the site's locale set. `zh` shadows the
