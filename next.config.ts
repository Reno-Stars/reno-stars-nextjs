...
const nextConfig: NextConfig = {
  ...
  // only header CF ignores — dead since the 2026-06 self-host migration, so HTML
  // has been fully origin-served). NOTE: on Cloudflare's default config HTML is
  // still bypassed even with this header — a Cache Rule marking these locale
  // paths "Eligible for cache" is required to actually offload the origin, and
  // would make edits surface at the edge within s-maxage (≤5min) rather than
  // instantly. Header kept correct so that rule works if/when enabled.
  async headers() {
