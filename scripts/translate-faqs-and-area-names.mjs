...
async function gtxTranslate(text, target, source = 'en') {
  // Use the same gtx free endpoint pattern as bulk-translate.py
...
async function backfillCityNames() {
  const areas = await sql`SELECT slug, name_en, localizations FROM service_areas`;
  console.log(`Backfilling city names for ${areas.length} areas...`);
  let n = 0;
  for (const a of areas) {
    const map = CITY_NAMES[a.slug];
    if (!map) { console.log(`  SKIP ${a.slug}: no transliteration map`); continue; }
    const locs = a.localizations || {};
    const updates = {};
    for (const loc of ['ru', 'ar', 'hi', 'fr']) {
      const key = 'name' + loc[0].toUpperCase() + loc.slice(1);
      if (locs[key]) continue; // already there
      updates[key] = map[loc];
    }
    if (Object.keys(updates).length === 0) continue;
    await sql`UPDATE service_areas SET localizations = COALESCE(localizations, '{}'::jsonb) || ${JSON.stringify(updates)}::jsonb, updated_at = NOW() WHERE slug = ${a.slug}`;
...
async function backfillFaqs() {
  const faqs = await sql`SELECT id, question_en, answer_en, localizations FROM faqs`;
  console.log(`\nBackfilling FAQs for ${faqs.length} rows (4 locales × 2 fields each)...`);
  let n = 0;
  for (const f of faqs) {
    const locs = f.localizations || {};
    const updates = {};
    for (const [loc, gtxLoc] of [['ru','ru'],['ar','ar'],['hi','hi'],['fr','fr']]) {
      ...
      if (!locs[qKey] && f.question_en) {
        ...
        try { updates[qKey] = await gtxTranslate(f.question_en, gtxLoc); process.stdout.write('ok\n'); }
        catch (e) { process.stdout.write('ERR ' + e.message.slice(0,50) + '\n'); }
        await new Promise(s => setTimeout(s, 200));
      }
      if (!locs[aKey] && f.answer_en) {
        process.stdout.write(`  ${f.id.slice(0,8)} a→${loc}... `);
        try { updates[aKey] = await gtxTranslate(f.answer_en, gtxLoc); process.stdout.write('ok\n'); }
        catch (e) { process.stdout.write('ERR ' + e.message.slice(0,50) + '\n'); }
        await new Promise(s => setTimeout(s, 200));
      }
    }
    if (Object.keys(updates).length === 0) continue;
    await sql`UPDATE faqs SET localizations = COALESCE(localizations, '{}'::jsonb) || ${JSON.stringify(updates)}::jsonb, updated_at = NOW() WHERE id = ${f.id}`;
    n += Object.keys(updates).length;
  }
