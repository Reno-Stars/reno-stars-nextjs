...
const dotenv = fs.readFileSync('/Users/renostars/.openclaw/workspace/reno-stars-nextjs-prod/.env.production.local', 'utf8');
const E = Object.fromEntries(dotenv.split('\n').filter(l => l.includes('=')).map(l => {
...
async function processAndUpload(srcPath, key) {
...
(async () => {
  ...
  const heroUrl = uploaded.find(u => u.hero).url;
  const afterShots = uploaded.filter(u => !u.isBefore);
  const beforeShots = uploaded.filter(u => u.isBefore);

  // Pair before/after by topic
  // 184 (before-tub)    ↔ 172 (after-tub-mirror) — tub area
  // 181 (before-shower) ↔ 176 (after-shower-1)   — shower area
  const PAIRS = [
    {
      ...
      before: uploaded.find(u => u.label === 'before-tub').url,
      before_alt_en: uploaded.find(u => u.label === 'before-tub').alt,
      after: uploaded.find(u => u.label === 'after-tub-mirror').url,
      after_alt_en: uploaded.find(u => u.label === 'after-tub-mirror').alt,
    },
    {
  ...
  const excerpt_en = "Champagne gold European-style luxury bathroom renovation in West Side Vancouver. Cream marble, frameless glass shower with built-in bench, French-paneled double vanity with arched ceiling, retained original tub reframed in marble, all-brass champagne gold hardware throughout. ~4-6 week build.";

  const excerpt_zh = "温哥华西区豪宅香槟金欧式风格主卫翻新。奶油大理石、双人无框玻璃淋浴房、拱形吊顶法式双人台盆、保留原有浴缸用大理石重新包边，全屋香槟金五金统一，工期约 4-6 周。";

  const tags_en = "champagne gold, european style, french luxury, cream marble, basketweave mosaic, frameless shower, double vanity, arched ceiling, brass sconces, west vancouver, luxury bathroom, master bath, marble walls, plantation shutters";
  ...
  const meta_desc_en = "West Side Vancouver luxury bath: cream marble, frameless glass shower, french-paneled double vanity, champagne gold hardware throughout. 4-6 wk build.";
  ...
  // Add additional after-only shots as image pairs (after only, no before)
