...
import {
  Clock, Home, TrendingUp, CheckCircle, AlertTriangle, ArrowRight, Paintbrush,
} from 'lucide-react';
...
export default function CabinetRefinishingCostGuidePage({ locale, projects }: CabinetRefinishingCostGuidePageProps) {
  ...
  const stats = useMemo(() => {
    ...
    if (budgets.length === 0) {
      ...
      return { min: 4_000, max: 15_000, avg: 8_000, count: projects.length };
    }
  ...
  const costFactors = [
    { key: 'cabinetCount', icon: CheckCircle },
    { key: 'material', icon: CheckCircle },
    { key: 'finishType', icon: CheckCircle },
    { key: 'hardware', icon: AlertTriangle },
    { key: 'kitchenSize', icon: AlertTriangle },
    { key: 'condition', icon: AlertTriangle },
  ];
    ...
    <main>
        ...
        <div className="max-w-4xl mx-auto text-center">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 max-w-3xl mx-auto">
            {[
              ...
              { label: t('stats.savings'), value: t('stats.savingsValue') },
      ...
      <section className="py-14 px-4 sm:px-6 lg:px-8" style={{ backgroundColor: SURFACE }}>
        <div className="max-w-4xl mx-auto">
          <h2 className="text-2xl md:text-3xl font-bold mb-2 text-center" style={{ color: TEXT }}>{t('comparison.title')}</h2>
          <p className="text-center mb-8" style={{ color: TEXT_MID }}>{t('comparison.subtitle')}</p>
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="rounded-xl p-5" style={{ backgroundColor: CARD, boxShadow: neu() }}>
              <div className="w-10 h-10 rounded-lg flex items-center justify-center mb-3" style={{ backgroundColor: STEP_GREEN_LIGHT }}>
                <Paintbrush size={20} style={{ color: STEP_GREEN }} />
              </div>
              <h3 className="font-bold mb-2" style={{ color: TEXT }}>{t('comparison.refinishing.title')}</h3>
              <ul className="text-sm space-y-1" style={{ color: TEXT_MID }}>
                <li>✓ {t('comparison.refinishing.pro1')}</li>
                <li>✓ {t('comparison.refinishing.pro2')}</li>
                <li>✓ {t('comparison.refinishing.pro3')}</li>
                <li>✗ {t('comparison.refinishing.con1')}</li>
              </ul>
            </div>
            <div className="rounded-xl p-5" style={{ backgroundColor: CARD, boxShadow: neu() }}>
              <div className="w-10 h-10 rounded-lg flex items-center justify-center mb-3" style={{ backgroundColor: STEP_ORANGE_LIGHT }}>
                <TrendingUp size={20} style={{ color: STEP_ORANGE }} />
              </div>
              <h3 className="font-bold mb-2" style={{ color: TEXT }}>{t('comparison.replacement.title')}</h3>
              <ul className="text-sm space-y-1" style={{ color: TEXT_MID }}>
                <li>✓ {t('comparison.replacement.pro1')}</li>
                <li>✓ {t('comparison.replacement.pro2')}</li>
                <li>✓ {t('comparison.replacement.pro3')}</li>
                <li>✗ {t('comparison.replacement.con1')}</li>
              </ul>
            </div>
          </div>
        </div>
