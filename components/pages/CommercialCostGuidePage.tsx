...
import {
  DollarSign, Clock, Building2, TrendingUp, CheckCircle, AlertTriangle, ArrowRight,
} from 'lucide-react';
...
export default function CommercialCostGuidePage({ locale, projects }: CommercialCostGuidePageProps) {
  ...
  const stats = useMemo(() => {
    return { min: 50_000, max: 1_000_000, avg: 250_000, count: projects.length };
  }, [projects]);
  ...
  const costFactors = [
    ...
    { key: 'hvac', icon: CheckCircle },
    { key: 'fireSafety', icon: AlertTriangle },
    { key: 'accessibility', icon: CheckCircle },
    { key: 'electrical', icon: AlertTriangle },
    { key: 'disruption', icon: CheckCircle },
  ];
