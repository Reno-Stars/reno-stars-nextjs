...
import {
  DollarSign, Clock, Home, TrendingUp, ArrowRight,
  Flame, DoorOpen, Zap, Droplets, ShieldCheck, FileCheck,
} from 'lucide-react';
...
export default function BasementSuiteCostGuidePage({ locale, projects, phone, share }: BasementSuiteCostGuidePageProps) {
  const t = useTranslations('guides.basementSuiteCost');

  const stats = useMemo(() => {
    // Basement renovation market estimates for Metro Vancouver (2025-2026)
    return { min: 60_000, max: 150_000, avg: 95_000, count: projects.length };
  }, [projects]);
  ...
  const costFactors = [
    { key: 'separateEntrance', icon: DoorOpen },
    { key: 'fireSeparation', icon: Flame },
    { key: 'plumbing', icon: Droplets },
    { key: 'electrical', icon: Zap },
    { key: 'egress', icon: ShieldCheck },
  ...
  return (
    <main>
      ...
      <section className="py-14 px-4 sm:px-6 lg:px-8" style={{ backgroundColor: SURFACE_ALT }}>
        <div className="max-w-4xl mx-auto">
          <h2 className="text-2xl md:text-3xl font-bold mb-2 text-center" style={{ color: TEXT }}>{t('roi.title')}</h2>
          <p className="text-center mb-8" style={{ color: TEXT_MID }}>{t('roi.subtitle')}</p>
          <div className="grid gap-4 sm:grid-cols-3">
            {(['rentalIncome', 'propertyValue', 'paybackPeriod'] as const).map((key) => (
              <div key={key} className="rounded-xl p-5 text-center" style={{ backgroundColor: CARD, boxShadow: neu() }}>
                <div className="text-2xl font-bold mb-2" style={{ color: GOLD }}>{t(`roi.${key}.value`)}</div>
                <h3 className="font-bold mb-1" style={{ color: TEXT }}>{t(`roi.${key}.title`)}</h3>
                <p className="text-sm" style={{ color: TEXT_MID }}>{t(`roi.${key}.description`)}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="py-14 px-4 sm:px-6 lg:px-8" style={{ backgroundColor: SURFACE_ALT }}>
        <div className="max-w-4xl mx-auto">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            {(['vancouver', 'burnaby', 'surrey', 'coquitlam'] as const).map((key) => (
              <div key={key} className="rounded-xl p-5" style={{ backgroundColor: CARD, boxShadow: neu() }}>
                <h3 className="font-bold mb-2" style={{ color: TEXT }}>{t(`municipalities.${key}.title`)}</h3>
                <p className="text-sm" style={{ color: TEXT_MID }}>{t(`municipalities.${key}.description`)}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="py-14 px-4 sm:px-6 lg:px-8" style={{ backgroundColor: SURFACE }}>
        <div className="max-w-4xl mx-auto">
          ...
          <ShareBar
            ...
            context={share}
            contentType="guide"
            itemId="basement-suite-cost-vancouver"
          />
        </div>
      </section>
      ...
      <section className="py-10 px-4 sm:px-6 lg:px-8" style={{ backgroundColor: SURFACE_ALT }}>
        <div className="max-w-4xl mx-auto text-center">
          ...
          <div className="flex flex-wrap justify-center gap-3">
            <Link href="/guides/basement-renovation-cost-vancouver" className="inline-flex items-center gap-1 text-sm font-semibold px-4 py-2 rounded-full transition-transform hover:scale-105" style={{ backgroundColor: CARD, boxShadow: neu(), color: GOLD }}>
              {t('related.basementGuide')} <ArrowRight size={14} />
            </Link>
      ...
      {/* FAQ — visible accordion matches FAQSchema; targets 'legal basement suite vancouver'
          long-tail questions including municipality rules, legalization, and tax/insurance */}
      <section className="py-14 px-4 sm:px-6 lg:px-8" style={{ backgroundColor: SURFACE_ALT }}>
        <div className="max-w-4xl mx-auto">
          <h2 className="text-2xl md:text-3xl font-bold mb-6 text-center" style={{ color: TEXT }}>
            Frequently Asked Questions — Basement Suite Cost Vancouver
          </h2>
