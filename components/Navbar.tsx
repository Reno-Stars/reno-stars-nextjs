      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
          <Link href="/" className="shrink-0">
            <OptimizedImage src={company.logo} alt={company.name} width={180} height={40} sizes="180px" priority placeholder="empty" className="h-10 w-auto object-contain" />
          </Link>
          ...
          <div className="hidden lg:flex items-center gap-0.5">
            ...
            <div className="relative" data-locale-switcher>
              <button
                ...
                className="px-3 py-2 text-sm font-semibold rounded-lg cursor-pointer transition-all duration-200 flex items-center gap-1.5"
              ...
              >
                ...
                {localeNames[locale as Locale] || (locale as string).toUpperCase()}
                <ChevronDown className="w-3 h-3" aria-hidden="true" />
              </button>
              {isLocaleMenuOpen && (
                <ul
                  role="listbox"
                  className="absolute right-0 top-full mt-2 min-w-[140px] rounded-lg overflow-hidden z-50"
        ...
        {isMenuOpen && (
          <div ref={menuRef} id="mobile-menu" className="lg:hidden pb-3 space-y-1 border-t" style={{ borderColor: `${TEXT}10` }}>
            ...
            <div className="pt-2 mt-2 border-t" style={{ borderColor: `${TEXT}10` }}>
              <div className="flex items-center gap-2 px-3 py-1.5 text-xs font-semibold uppercase opacity-60" style={{ color: TEXT }}>
                <Globe className="w-3.5 h-3.5" /> {t('nav.switchLanguage')}
              </div>
              {locales.map((l) => (
                <Link key={l} href={pathname || '/'} locale={l} onClick={closeMenu}
                  className="block py-2 px-3 text-base font-medium cursor-pointer rounded-lg"
                  style={{ color: TEXT, fontWeight: l === locale ? 700 : 500 }}>
                  {localeNames[l]}
                </Link>
              ))}
