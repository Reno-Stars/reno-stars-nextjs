...
- **ImageUrlInput** (`components/admin/ImageUrlInput.tsx`): URL input with drag-and-drop upload, image preview, and tooltip. Accepts optional `slug`, `imageRole` (default `'hero'`), and `disabled` props. SEO-friendly upload naming (`{slug}-{imageRole}.{ext}`). Upload area uses a detached file picker (`document.createElement('input')`) to bypass `<fieldset disabled>` and is hidden entirely in view mode.
