...
lib/
  ai/
    ...
    social-post-generator.ts # AI social post generation for 3 platforms (Instagram, Facebook, Xiaohongshu)
  db/
...
| `OPENAI_API_KEY` | No | OpenAI API key for AI content optimization in admin (blog, project forms, site forms, social post generation) |
...
- `social_media_posts` — social media campaigns for 3 platforms (Instagram, Facebook, Xiaohongshu) with bilingual content, JSONB `selectedImageUrls`, 3 nullable source FKs with CHECK constraint, `social_post_status` enum
...
- **Admin** (`app/admin/`): Auth-protected dashboard with CRUD for all 14 content types: projects, blog, social posts, contacts, company, services, social links, service areas, gallery, trust badges, FAQs, showroom, about sections, partners. Uses `components/admin/` (DataTable, HouseStack, ProjectForm, SiteForm, ImagePairEditor, Tooltip, DragHandleIcon, ConfirmDialog, DashboardShell, SubmitButton, EditModeToggle, FormField, FormAlerts, AdminLocaleProvider, etc.) and `app/actions/admin/`. Mobile-responsive via `app/admin/admin-responsive.css` (CSS-only overrides with `className` hooks on form grids/cards/headers).
...
- **Image upload** (presigned S3 URL flow): All admin image uploads (hero images, before/after pairs) use presigned S3 URLs to bypass Vercel's body size limit. Flow: client calls `uploadImageDirect()` from `lib/admin/upload-client.ts` → requests presigned PUT URL from `POST /api/admin/upload` → uploads directly from browser to S3. API route (`app/api/admin/upload/route.ts`) validates auth, file metadata (max 50 MB, JPEG/PNG/WebP/SVG/GIF), generates S3 key, and returns `{ presignedUrl, publicUrl }`. Presigned URL includes `ContentLength` condition to enforce declared file size. Shared constants in `lib/admin/upload-constants.ts` (`MAX_IMAGE_SIZE`, `MAX_IMAGE_SIZE_LABEL`, `ALLOWED_IMAGE_TYPES`) used by both API route and client. `customKey` sanitized server-side (`/[a-z0-9-]/` only, max 200 chars); falls back to `{timestamp}-{random8}` naming. Presigned URLs expire after 10 minutes.
- **S3 client** (`lib/admin/s3.ts`): Shared S3 client singleton with lazy initialization (caches `S3Client` or `null`). Configured with `requestChecksumCalculation: 'WHEN_REQUIRED'` to disable AWS SDK v3 automatic CRC32 checksums (R2 rejects them as unsigned headers on presigned URLs). Exports `getS3Client()`, `S3_BUCKET` constant, and `MIME_TO_EXT` map. Used by image upload API route, batch upload API route, and batch processor.
- **Batch upload** (`app/admin/(dashboard)/batch-upload/`): ZIP file upload that auto-creates sites, projects, image pairs, external products, and blog posts. Two modes: **Sites** (whole-house with site containers) and **Standalone** (individual projects under `STANDALONE_SITE_SLUG`). Uses presigned S3 URL upload flow to bypass the proxy body size limit: client sends JSON metadata (fileName, fileSize, generateBlog, mode) to the API route, receives a presigned PUT URL (`@aws-sdk/s3-request-presigner`, 10-minute expiry), uploads ZIP directly to S3 via `fetch()` PUT at `temp/batch/{jobId}.zip`, then triggers processing. Client starts polling immediately after job creation (before S3 upload completes) so the UI always transitions to results phase; on S3 failure it returns to upload phase, but if only the process trigger fails polling continues. Processing runs via `after()` from `next/server` to survive Vercel response send (`maxDuration = 60` for Hobby tier). Branches early based on mode: **Sites mode** pipeline: `parseZip()` → upload images (batches of 15 via `UPLOAD_BATCH_SIZE`, per-site slug prefix) → AI metadata (`optimizeSiteDescription` + `optimizeProjectDescription` in batches of 10 via `AI_METADATA_BATCH_SIZE`; no alt text generation — uses `FALLBACK_ALT` directly) → save to DB (sites + `site_image_pairs` + projects + `project_image_pairs` + external products, with orphan cleanup) → optional `generateBlogFromSite()` → cleanup temp ZIP. **Standalone mode** pipeline: `parseZipStandalone()` → upload images → AI metadata (`optimizeProjectDescription` in batches of 10) → find or create standalone site container via `ensureStandaloneSite()` → save projects (sequential `displayOrderInSite`) → optional `generateBlogFromProject()` → cleanup. Stale job detection in the GET polling endpoint: `STALE_PROCESSING_MS` (2 min) marks stuck processing jobs as `partial`/`failed`; `STALE_PENDING_MS` (30 min) catches abandoned pending jobs. Timeout errors use `__TIMEOUT_*__` marker strings resolved client-side by `resolveErrorLabel()` into bilingual display text. Shared helpers: `collectProjectImages()`, `collectAfterImageUrls()`, `generateBlogsForEntities()`, `finalizeJob()`, `cleanupTempZip()`. AI extracts all metadata including service type, PO number, budget, duration from freeform notes. AI-detected service type overrides folder-name heuristic when available. ZIP parser uses shared `extractFilesFromZip()` for file tree building. Sites mode supports nested layout (top folder = site, subfolders = projects, root images = site-level pairs in `site_image_pairs`), single-folder layout (project only, no site pairs), and flat layout (all at root). Standalone mode treats each top-level folder as a project (subfolders flattened into parent). Before/after pairing accepts both hyphens and spaces (`before-1.jpg` or `Before 1.jpg`), case-insensitive. `hero.jpg` becomes hero image; `product-N.jpg` (1-based) matched to Nth entry in `products.txt` (sparse indices supported); unmatched names become after-only pairs. Products files (`products.txt`, `links.txt`, `external.txt`) use format `URL | Label EN | Label ZH` with `#` comment lines. Shared `insertExternalProducts()` helper handles both site-level and project-level product insertion. Uses `batch_upload_jobs` table with jsonb columns for type-safe progress tracking (`BatchJobOptions` includes `generateBlog` and `mode`). `BatchUploadClient.tsx` has tab bar for mode selection (disabled during processing) + 3-phase UI (upload → processing → results) with mode-adaptive help content, step indicators, progress bar, error details, and locale-aware "Download Example ZIP" links (`public/example-batch-upload-{en|zh}.zip` and `public/example-batch-upload-standalone-{en|zh}.zip`). Help section content (folder structure, notes example, products example) adapts to selected mode, fully bilingual via admin translation keys. All API response types are validated at runtime (no `as` type casts).
...
- **AI field generators**: `AIFieldGenerator<T>` (generic component — notes textarea + generate button with error/success feedback, `mountedRef` cleanup). `AIProjectGenerator` and `AISiteGenerator` are thin wrappers providing entity-specific server actions and translations. Both generate service type, slug, bilingual titles, descriptions, badges, excerpts, PO number, budget, duration, and full SEO metadata from freeform notes. AI detects service type from notes content (e.g., 卫生间 → bathroom); `ProjectForm` auto-sets the service type dropdown from AI response. Slugs are descriptive (scope + style + location, not generic). All fields are AI-extracted (no hard regex extraction). Positioned at the top of their respective forms (before slug/title fields). Response validated with Zod schemas in `lib/ai/content-optimizer.ts`.
...
- **AI social post generation** (`lib/ai/social-post-generator.ts` + `app/actions/admin/social-posts.ts`): Generates bilingual content for Instagram, Facebook, and Xiaohongshu from blog/project/site data via GPT-4o. Each campaign row stores all 3 platforms' content. Form uses WAI-ARIA Tabs pattern for platform switching. Image picker fetches source images (hero + image pairs + child project images). `StatusCell` uses optimistic state with error rollback. Zod schema validates all 13 string fields with `.min(1)`. Server actions use shared `groupBy()` helper from `queries.ts` for child project relation grouping.
