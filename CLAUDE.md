...
- **Batch upload** (`app/admin/(dashboard)/batch-upload/`): ZIP file upload that auto-creates sites, projects, image pairs, external products, and blog posts. Uses presigned S3 URL upload flow to bypass the proxy body size limit: client sends JSON metadata (fileName, fileSize, generateBlog) to the API route, receives a presigned PUT URL (`@aws-sdk/s3-request-presigner`, 10-minute expiry), uploads ZIP directly to S3 at `temp/batch/{jobId}.zip`, then triggers processing. Client polls job status every 2s. Processing runs via `after()` from `next/server` to survive Vercel response send. Pipeline: extract ZIP (async `fflate`) → upload images to S3 (batches of 3, per-site slug prefix) → generate AI metadata (site/project descriptions + alt text in parallel batches of 5) → save to DB (sites + `site_image_pairs` + projects + `project_image_pairs` + external products with matched product images, with orphan cleanup checking both child projects and site image pairs) → optional blog generation → delete temp ZIP from S3. ZIP parser supports nested layout (top folder = site, subfolders = projects, root images = site-level pairs in `site_image_pairs`), single-folder layout (project only, no site pairs), and flat layout (all at root). Before/after pairing accepts both hyphens and spaces (`before-1.jpg` or `Before 1.jpg`), case-insensitive. `hero.jpg` becomes hero image; `product-N.jpg` (1-based) matched to Nth entry in `products.txt` (sparse indices supported — skip an index for products with no image); unmatched names become after-only pairs. Products files (`products.txt`, `links.txt`, `external.txt`) use format `URL | Label EN | Label ZH` with `#` comment lines. Shared `insertExternalProducts()` helper handles both site-level and project-level product insertion. Uses `batch_upload_jobs` table with jsonb columns for type-safe progress tracking. `BatchUploadClient.tsx` has 3-phase UI (upload → processing → results) with step indicators, progress bar, error details, and a locale-aware "Download Example ZIP" link (`public/example-batch-upload-{en|zh}.zip`). Help section content (notes example, products example) is fully bilingual via admin translation keys.
